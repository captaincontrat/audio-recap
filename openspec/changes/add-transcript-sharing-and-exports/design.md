...
- `txt`: plain-text download with markdown syntax stripped
- `pdf`: PDF generated from the client-side rendered export document
- `docx`: DOCX generated from the same client-side export representation

The frontend may use an intermediate parsed markdown document model to keep formatting consistent across all formats, but the backend contract remains markdown-only.
...
- [Client-side `pdf` and `docx` conversion can diverge visually] -> Use one intermediate export document model on the frontend so all formats derive from the same assembled markdown content.
...
5. Implement frontend conversion from canonical markdown into `md`, `txt`, `pdf`, and `docx`.
