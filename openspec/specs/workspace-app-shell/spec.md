...
Defines the shared authenticated shell for workspace-scoped private routes and for authenticated user-scoped account-settings routes (`/account/security`, `/account/close`). The shell wraps the workspace overview, the transcript library, the transcript detail page, the dedicated meeting-submission page, the dedicated meeting-status page, and the in-shell account-settings pages in a single layout that preserves workspace identity through workspace-aware sidebar navigation, surfaces a thin header ribbon with a reserved (non-input) workspace search position and a shadcn `CommandDialog`-based pre-launch command surface, renders a sticky breadcrumb band directly above page content, and exposes workspace-scoped upload chrome — a global drag-and-drop target inside the shell, an explicit header upload control, a drop-then-confirm handoff, and a persistent workspace-scoped upload manager that carries submissions from local upload phases into the existing transcript-processing lifecycle. On workspace-scoped routes the shell's current-workspace context comes from the explicit workspace route context defined by `workspace-foundation`; on authenticated account-settings routes hosted inside the shell the current-workspace context is resolved using the same default-workspace resolution defined by `workspace-foundation` for authenticated entry without an explicit workspace destination. The shell inherits the role-based access model and archived-workspace behavior from `workspace-foundation` rather than inventing a separate access model. Does not host public share routes, authentication routes, the minimal account gates `/account/recent-auth` and `/account/closed`, or the authenticated dashboard entry point (`/dashboard`).
