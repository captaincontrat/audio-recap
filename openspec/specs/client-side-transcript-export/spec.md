# client-side-transcript-export Specification

## Purpose

Defines downloadable transcript exports for authenticated workspace users with read access to completed transcript records. The backend remains the single source of canonical markdown for display title, recap, and transcript content; the frontend assembles one export document in the order display title, recap section, transcript section and converts it locally into `md`, `txt`, `pdf`, and `docx` without any server-side conversion, backend-generated export files, or HTML payloads. Export actions are workspace-scoped read actions available to `read_only`, `member`, and `admin` users while the workspace is active under `workspace-archival-lifecycle`, gated to `completed` status, and consistently refused when the current membership cannot read the transcript. Public share surfaces defined by `public-transcript-sharing` remain read-only and MUST NOT expose these export actions. Client-side conversion failures surface as user-visible errors without mutating the transcript record or its canonical markdown fields.
