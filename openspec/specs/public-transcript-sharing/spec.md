# public-transcript-sharing Specification

## Purpose

Defines active-workspace public sharing for completed transcript records: workspace role-based enable/disable/rotate actions for `member` and `admin` users, a double-UUID public URL design that avoids exposing internal transcript identifiers, read-only privacy-minimal public rendering that surfaces only the display title, canonical recap markdown, and canonical transcript markdown, workspace-visible share-state organization controls in the private transcript library, and a uniform unavailable behavior for disabled, rotated, missing, or otherwise invalid public links. Archive-driven availability of public links, including the post-restore requirement for fresh share management before previously enabled links become active again, is owned by `workspace-archival-lifecycle` and honored on every public share surface defined here.
