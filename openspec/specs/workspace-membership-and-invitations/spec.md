# workspace-membership-and-invitations Specification

## Purpose

Defines how team workspaces grow their membership after creation: admin-managed add, remove, and role-change flows for existing accounts; invitation issuance by email for people who may or may not have an account yet; and the invitation lifecycle rules (7-day expiration, revoke, resend with fresh token, single-use acceptance, and matching-email acceptance) that govern those invitations. This capability is layered on top of `workspace-foundation` (which owns workspace types, membership roles, and the eligible-active-admin invariant) and defers archive-driven invitation invalidation and post-restore re-invite behavior to `workspace-archival-lifecycle`. Personal workspaces are explicitly outside this capability: they do not support invitations or extra memberships.
