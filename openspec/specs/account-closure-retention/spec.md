# account-closure-retention Specification

## Purpose

Defines the V1 account lifecycle after sign-up: closure or deactivation, a 30-day self-service reactivation window, expiry-driven permanent deletion, generic deleted-user attribution for retained workspace-owned resources, and last-eligible-active-admin handoff rules in a collaborative workspace model. Sits on top of the `core-account-authentication` and `account-security-hardening` foundations — recent-auth and fresh second-factor prerequisites are reused from those capabilities to gate closure, and workspace invariants from `workspace-foundation` / `workspace-membership-and-invitations` constrain when closure may proceed.
